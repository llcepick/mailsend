...
int process_oneline_messages( const char *boundary )
{
	...
	for( l = oneline_attachment_list; l; l = l->next )
	{
		...
		{
			/* encode the message to base 64 and write to socket */
...
int include_msg_body( void )
{
	...
	// Read through once looking for only CR or only LF


	// We need to read the file into the buffer - but need to handle a line
	// at a time - so that we can guarantee that the line endings are all
	// CRLF - so simple fgets does not work well at a time...

	while( fgets( buf, bufsz, fp ) )
	{
		// We have same and correct EOL pattern...
		// We have individual EOL split...
		write_to_socket( buf );
		if( g_show_attachment_in_log )
		{
			showVerbose( "[C] %s", buf );
